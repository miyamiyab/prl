...
import { ensureDefaultIssuer, listIssuers, getIssuer, getIssuerPrivateKey, getIssuerPublicKey, getIssuerPublicJwk, createManagedIssuer, registerExternalIssuer, } from "./issuerStore.js";
const app = express();
...
function holderDidPkh(chainId, address) {
...
app.post("/register-issuer", async (req, res) => {
    try {
        ...
        if (!issuerId || typeof issuerId !== "string")
            return res.status(400).json({ error: "issuerId required" });
        if (address && (typeof address !== "string" || !isHexAddr(address)))
            return res.status(400).json({ error: "address must be 0x..." });
...
app.post("/register-external-issuer", async (req, res) => {
    try {
        ...
        if (!publicJwk || typeof publicJwk !== "object")
            return res.status(400).json({ error: "publicJwk required" });
...
app.get("/requests", async (req, res) => {
    try {
        ...
        let requests = await readJson(REQUESTS_PATH, []);
        if (status)
            requests = requests.filter((r) => r.status === status);
        if (issuerId)
            requests = requests.filter((r) => r.issuerId === issuerId);
...
app.post("/requests/:id/issue", async (req, res) => {
    ...
    try {
        ...
        if (r.status !== "requested")
            return res.status(400).json({ error: "request is not in requested status" });
        ...
        await writeJson(REQUESTS_PATH, requests);
        const vcs = await readJson(VCS_PATH, []);
        const published = {
            id: randomUUID(),
...
app.get("/vcs", async (req, res) => {
    try {
        const issuerId = req.query.issuerId;
        let vcs = await readJson(VCS_PATH, []);
        if (issuerId)
            vcs = vcs.filter((v) => v.issuerId === issuerId);
        return res.json({ vcs });
    }
    catch (e) {
        return res.status(500).json({ error: e?.message ?? String(e) });
...
app.post("/verify", async (req, res) => {
    try {
        ...
        if (!vcJwt || typeof vcJwt !== "string")
            return res.status(400).json({ error: "vcJwt required" });
        ...
        if (!issuerDid || typeof issuerDid !== "string")
            return res.status(400).json({ error: "issuer DID not found in token" });
        const issuers = await listIssuers();
        const issuer = issuers.find((x) => x.did === issuerDid);
        if (!issuer)
            return res.status(404).json({ error: "issuer not registered on platform" });
